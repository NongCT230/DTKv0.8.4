...
#fub poulin&haddad2012 model data importer


Class.DataImporter.dreamtk.FubPoulinHaddad2012 <- R6Class("Class.DataImporter.dreamtk.FubPoulinHaddad2012",
  ...
  private = list(
    ...
    loadModelDataFromMySQL = function ( dreamtk.db = NULL, chemicals = "*"){
      ...
      loginfo("Importing dreamtk v1.0 Poulin&Haddad2012 model data from MySQL database...");
    ...
    loadModelDataFromRData = function ( file.path = NULL, chemicals = "*"){
      ...
      if (is.null(chemicals)){
        e <- "DataImporter.dreamtk.FubPoulinHaddad2012$loadModelDataFromRData(): Invalid filter list given. Use '*' to get all values.";
        logerror( e );
        stop(e);
      }
      ...
      if (is.null(private$model_data_rdata)){
        ...
        loginfo("Importing dreamtk v1.0 FubPoulinHaddad2012 model data from RData file...");
        table <- Class.DataImporter.dreamtk.tableinfo$kowtablename;
        tablevars <- Class.DataImporter.dreamtk.tableinfo$kowtablevars;
  ...
  public = list(
    ...
    #constructor
    initialize = function(){
      self$classname <- "Class.DataImporter.dreamtk.FubPoulinHaddad2012";
    ...
    loadModelData = function ( chemicals = "*"){
      if (private$lock_data){
        w <- "DataImporter.dreamtk.FubPoulinHaddad2012$loadModelData(): data not imported from source specified by $setSource() since it is locked by $setModelData(). Use $resetModelData() to unlock.";
        return ( logwarn( w ) );
      }

      #Removed the MySQL database choice #YANIC
    ...
    setModelData = function ( modeldata = NULL ){
      ...
      if (all(vars_exist)){
        ...
        w <- "DataImporter.dreamtk.FubPoulinHaddad2012$setModelData(): data set and locked. Use $resetModelData() to unlock, to be able to $loadModelData() from source specified by $setSource().";
        logwarn( w );
      } else {
        e <- "DataImporter.dreamtk.FubPoulinHaddad2012$setModelData(): Data missing required columns.";
        logerror( e );
        stop(e);
      }
    },
    ...
    resetModelData = function (){
      ...
      w <- "DataImporter.dreamtk.FubPoulinHaddad2012$resetModelData(): data reset and unlocked. $loadModelData() is now functional.";
      logwarn( w );
    },
